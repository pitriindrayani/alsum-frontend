...
export default function AbsensiGuru() {
  ...
  const GetResponseData2 = async () => {
    try {
      ...
      const response = await API.post(`/api/schedule-teachers/status`, {
       ...
       period_start: `${form2?.period_start}`,
       period_end: `${form2?.period_end}`,
       period_start_puasa: `${form2?.period_start_puasa}`,
       period_end_puasa: `${form2?.period_end_puasa}`,
       level: storageLevel,
  ...
  function generateDates( getData.filter) {
    API.get(`/api/periods?page=${page}&limit=${limit}&ascending=${ascending}`, fetchParams),

    getData.filter
    const startDate = new Date(getData.filter.date paling awal dan masih di range tanggal periods.period_start_puasa === null ? period_start:period_start_puasa && dan masih di range tanggla terakhir periods.period_end_puasa === null? period_end:period_end_puasa);
    const endDate = new Date(getData.filter.date paling akhir dan masih di range tanggal periods.period_start_puasa === null ? period_start:period_start_puasa && dan masih di range tanggla terakhir periods.period_end_puasa === null? period_end:period_end_puasa);
    const dates = [];
   ...
   function generateDatesNext(periods) {
    const startDate = new Date(periods.period_start_puasa === null ? period_start:period_start_puasa);
    const endDate = new Date(periods.period_end_puasa === null? period_end:period_end_puasa);
    const dates = [];
  ...
  useEffect(() => {
    if (getData && getData.length !== 0){
      getData.filter
      const generatedDates = generateDates(getData.filter);
      setData(generatedDates);
    } else{
       API.get(`/api/periods?page=${page}&limit=${limit}&ascending=${ascending}`, fetchParams),
      const generatedDatesNext = generateDatesNext(periods.);
      setData(generatedDatesNext);
    }
  }, [ getData]);  
