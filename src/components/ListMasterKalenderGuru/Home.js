...
export default function Login() {
  document.title = "List Kalender Guru";
  ...
  const GetResponseData = async () => {
    try {
      // e.preventDefault();
      // setLoading(true)
      const response = await API.get(`/api/schedule-times?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}&branch=${storageBranch}&level=${storageLevel}`,fetchParams)
  ...
  const deleteById = async (id) => {
    Swal.fire({
      ...
      if(result.isConfirmed) {
        const response =  await API.delete(`/api/schedule-times/${id}`,fetchParams);
      ...
      {isTabletOrMobile ? 
        <div style={{ paddingLeft: "0px", width: "100%", borderBottom: "5px solid #EEEEEE", display: "", padding: "0px 0px 10px 0px" }}>
          <Col xl="6" style={{fontSize:"16px",display:"flex", justifyContent:"center", alignItems:"center", padding:"7px", color:"white", backgroundColor:"#2e649d"}}>
              <FaListAlt style={{marginRight:"5px"}}/>List Kalender Shift Guru
          </Col>
            ...
            <table className="table dt-responsive nowrap w-100" id="basic-datatable">
              <thead>
                <tr style={{backgroundColor: isTabletOrMobile? "white" : "white", borderBottom:"1px solid rgb(214, 214, 214)"}}>
                  ...
                  <th style={{ fontFamily: "Poppins", fontSize: "12px", color: "#525252", border: "none", textAlign: ""}}>WAKTU</th>
                  ...
                  {getData.map((user,index) => (
                    <tr key={index} style={{fontFamily:"Poppins", fontSize:"11px", textAlign:"center"}}>
                      ...
                      <td style={{ lineHeight: "2", minWidth:"" }}>
                        <div style={{display:"flex",textAlign:"left"}}>
                          <div>
                            ...
                            <FormatHolidayDate props={user.date_in}/>
                            : 
                            <>
                              <FormatHolidayDate props={user.date_in}/> - <FormatHolidayDate props={user.date_out}/>
                            </>}
                          </div> 
                        </div>
                      </td>
                      ...
                      <td style={{ lineHeight: "2" }}>
                        <div style={{display:"flex",textAlign:"left"}}>
                          <div>
                            {user?.time_in.split(":")[0]}:{user?.time_in.split(":")[1]} - {user?.time_out.split(":")[0]}:{user?.time_out.split(":")[1]}
                          </div> 
                        </div>
                      </td>
