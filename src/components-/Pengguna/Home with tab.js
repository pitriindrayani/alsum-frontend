...
export default function Login() {
  ...
  const GetResponseData = async () => {
    try { 
      // e.preventDefault();
      // setLoading(true)
      const response = await API.get(`/user-service/privileges/roles?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}`,fetchParams)
  ...
  const GetResponseDataUser = async () => {
    try { 
      // e.preventDefault();
      // setLoading(true)
      const response = await API.get(`/user-service/privileges/users?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}`,fetchParams)
  ...
  const deleteById = async (id) => {
    Swal.fire({
      ...
      if(result.isConfirmed) {
        const response =  await API.delete(`/user-service/privileges/users/${id}`,fetchParams);
  ...
  return (
    <div className="body" >
      ...
      {isTabletOrMobile ? 
        <>
          <div className="title-page">
            <h6> <FontAwesomeIcon icon={faUsers} /> Data Pengguna</h6>
          </div> 
        ...
        <>
          <div className="title-page">
            <h5> <FontAwesomeIcon icon={faUsers} /> Daftar Pengguna </h5>
          </div>
      ...
      <div className="body-content">
        ...
        >
          <Tab eventKey="home" title="Semua Akun" className="nav-item">
        ...
        {/* TABLE */}
                  ...
                  <tbody>
                      {getDataUser.map((user,index) => (
                        <tr key={index}>
                          ...
                          <td >
                            <button className="button-edit button-table" onClick={() => viewModalUpdate(user?.id, user
          ...
          <Tab eventKey="profile" title="Jenis Akun Role">
            {/* <h6 className="mt-4">Jenis Akun Role</h6>
            <hr /> */}
