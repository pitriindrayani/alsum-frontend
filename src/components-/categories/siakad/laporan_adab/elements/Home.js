...
export default function Elements() {
  ...
  const GetResponseData = async () => {
    try {
      // e.preventDefault();
      setLoading(true)
      const response = await APILA.get(`/api/elements?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}`,fetchParams)
  ...
  const deleteById = async (id) => {
    Swal.fire({
      ...
      if(result.isConfirmed) {
        const response =  await APILA.delete(`/api/elements/${id}`,fetchParams);
  ...
  const viewModalUpdate = (id, elements) => {
    setModalUpdate(true)
    setId(id)
    setDataUpdate(elements)
  }

  const navigateSubElement = (id, slug_name) => {
    navigate("/sub-elements/" + id  + "/" + slug_name)
  }
        ...
        {isTabletOrMobile ? 
          <>
            <div className="title-page">
              <h6> <FontAwesomeIcon icon={faListUl} /> Data Element</h6>
            </div> 
                  
            <div className="ml-auto">
              <button onClick={viewModalAdd} className="btn btn-create"> <FontAwesomeIcon icon={faPlus} />Element</button>
            </div>
          </>
          : 
          <>
            <div className="title-page">
              <h5> <FontAwesomeIcon icon={faListUl} /> Data Element </h5>
            </div>
            
            <div className="ml-auto">
              <button onClick={viewModalAdd} className="btn btn-create"> <FontAwesomeIcon icon={faPlus} /> Tambah Element</button>
            </div>
          </>
        }
      </div> 
        ...
        <div className="breadcrumb-header mb-4" style={{fontSize:"14px" }}>
          <nav aria-label="breadcrumb">
            <ol className="breadcrumb">
              ...
              <li className="breadcrumb-item active" aria-current="page"> Element</li>
            </ol>
          </nav>
        </div>
            ...
            <div className="body-table" >
              <div>
                <table className="table dt-responsive nowrap w-100" id="basic-datatable">
                  ...
                  <tbody>
                    {getData.map((elements,index) => (
                      <tr key={index}>
                        <td style={{ lineHeight: "2" }}> {(page - 1) * 10 + (index + 1)} </td>  
                        <td style={{ lineHeight: "2" }}> {elements.name_dimensi} </td>
                        <td style={{ lineHeight: "2" }}> {elements.name_element} </td>
                        
                        <td style={{ lineHeight: "2" }}> {elements.description} </td>
                        <td style={{ lineHeight: "2" }}>
                          <div onClick={() => navigateSubElement(elements?.id, elements?.slug_name)}  style={{backgroundColor:"#4747AC", borderRadius:"3px", cursor:"pointer", width:"70px", color: "white", paddingLeft: "8px" }}><FaArrowAltCircleRight /> Detail </div>
                        </td>
                        <td >
                          <button className="button-edit button-table" onClick={() => viewModalUpdate(elements?.id, elements)} > <FontAwesomeIcon icon={faPenToSquare} /></button>
                          <button className="ml-2 button-delete button-table" onClick={() => {deleteById(elements?.id)}}> <FontAwesomeIcon icon={faTrash} /></button>                           
                        </td>
                      </tr>
                    ))}
                  </tbody>
