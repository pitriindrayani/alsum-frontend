...
export default function Rapor() {
  ...
  const [getDataElement, setGetDataElement] = useState([]);
  ...
  const GetResponseData = async () => {
    try {
      ...
      const response = await APILA.get('api/element-recaps-rapot/rapot', {
        params: {
      ...
      if (response?.status === 200) {
        ...
        setGetData(respData);
        setGetRapor(respData);

        // Build elements list for tabs from returned data (unique by element id)
        try {
          const map = new Map();
          respData.forEach(student => {
            (student.dimensi || []).forEach(dim => {
              (dim.data_elements || []).forEach(el => {
                const id = el.ysb_element_id || el.id || el.element_id;
                if (!map.has(id)) {
                  map.set(id, {
                    id: id,
                    name_element: el.name_element || el.name || el.title || '',
                    description: el.name_description || el.description || '',
                    raw: el
                  });
                }
              });
          ...
          setGetDataElement(Array.from(map.values()));
        } catch (e) {
          console.error('build elements error', e);
        }
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        try {
          ...
          setGetDataElement(Array.from(map2.values()));
        } catch (e) {
          console.error('build elements error', e);
        }
