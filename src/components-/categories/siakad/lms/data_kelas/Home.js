...
export default function DataKelas() {
  ...
  const GetResponseData = async () => {
    try {
      // e.preventDefault();
      setLoading(true)
      const response = await APIMS.get(`/api/kelas?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}`,fetchParams)
  ...
  const deleteById = async (id) => {
    Swal.fire({
      ...
      if(result.isConfirmed) {
        const response =  await APIMS.delete(`/api/kelas/${id}`,fetchParams);
        ...
        {isTabletOrMobile ? 
            <>
                <div className="title-page">
                  <h6> <FontAwesomeIcon icon={faListUl} /> Data Kelas</h6>
                </div> 
                
                <div className="ml-auto">
                  <button onClick={viewModalAdd} className="btn btn-create"> <FontAwesomeIcon icon={faPlus} />Kelas</button>
                </div>
            </>
            : 
            <>
                <div className="title-page">
                  <h5> <FontAwesomeIcon icon={faListUl} /> Data Kelas </h5>
                </div>
          
                <div className="ml-auto">
                  <button onClick={viewModalAdd} className="btn btn-create"> <FontAwesomeIcon icon={faPlus} /> Tambah Kelas</button>
                </div>
            </>
        }
      </div> 
                ...
                <div className="body-table" >
                    <div>
                        <table className="table dt-responsive nowrap w-100" id="basic-datatable">
                            ...
                            <tbody>
                                {getData.map((kelas,index) => (
                                    <tr key={index}>
                                        <td style={{ lineHeight: "2" }}> {(page - 1) * 10 + (index + 1)} </td>  
                                        <td style={{ lineHeight: "2" }}> {kelas.number_kelas} </td>
                                        <td style={{ lineHeight: "2" }}> {kelas.name_kelas} </td>
                                        <td style={{ lineHeight: "2" }}> {kelas.name_branch} </td>
                                        <td style={{ lineHeight: "2" }}> {kelas.name_school} </td>
                                        <td>
                                            <button className="button-edit button-table" onClick={() => viewModalUpdate(kelas?.id, kelas)} > <FontAwesomeIcon icon={faPenToSquare} /></button>
                                            <button className="ml-2 button-delete button-table" onClick={() => {deleteById(kelas?.id)}}> <FontAwesomeIcon icon={faTrash} /></button> 
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
