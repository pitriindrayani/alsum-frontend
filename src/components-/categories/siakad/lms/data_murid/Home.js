...
export default function DataMurid() {
  ...
  const GetResponseData = async () => {
    try {
      // e.preventDefault();
      setLoading(true)
      const response = await APILA.get(`/api/semester-assignments?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}&branch=${storageBranch === null? "" : storageBranch}&level=${storageLevel},`,fetchParams)
  ...
  const deleteById = async (id) => {
    Swal.fire({
      ...
      if(result.isConfirmed) {
        const response =  await APILA.delete(`/api/semester-assignments/${id}`,fetchParams);
  ...
  const viewModalUpdate = (id, murid) => {
    setModalUpdate(true)
    setId(id)
    setDataUpdate(murid)
  }
        ...
        <div className="body-header d-flex">
            ...
            {isTabletOrMobile ? 
                <>
                    ...
                    <h6> <FontAwesomeIcon icon={faListUl} /> Data Murid</h6>
                    </div> 
                    
                    <div className="ml-auto">
                    <button onClick={viewModalAdd} className="btn btn-create"> <FontAwesomeIcon icon={faPlus} />Murid</button>
                    </div>
                </>
                : 
                <>
                    <div className="title-page">
                    <h5> <FontAwesomeIcon icon={faListUl} /> Data Murid 1 </h5>
                    </div>
            
                    <div className="ml-auto">
                    <button onClick={viewModalAdd} className="btn btn-create"> <FontAwesomeIcon icon={faPlus} /> Tambah Murid</button>
                    </div>
                </>
            }    
        </div> 
                    ...
                    <div className="body-table" >
                        <div>
                            <table className="table dt-responsive nowrap w-100" id="basic-datatable">
                                ...
                                <tbody>
                                    {getData.map((murid,index) => (
                                    <tr key={index}>
                                        <td style={{ lineHeight: "2" }}> {(page - 1) * 10 + (index + 1)} </td>  
                                        <td style={{ lineHeight: "2" }}> {murid.name_student} </td>
                                        <td style={{ lineHeight: "2" }}> {murid.nisn} </td>
                                        <td style={{ lineHeight: "2" }}> {murid.name_teacher} </td>
                                        <td style={{ lineHeight: "2" }}> {murid.number_kelas} - {murid.name_kelas} </td>
                                        <td style={{ lineHeight: "2" }}> {murid.name_school} </td>
                                        <td style={{ lineHeight: "2" }}> {murid.name_branch} </td>
                                        <td>
                                        <button className="button-edit button-table" onClick={() => viewModalUpdate(murid?.id, murid)} > <FontAwesomeIcon icon={faPenToSquare} /></button>
                                        <button className="ml-2 button-delete button-table" onClick={() => {deleteById(murid?.id)}}> <FontAwesomeIcon icon={faTrash} /></button>          
                                        </td>
                                    </tr>
                                    ))}
                                </tbody>
