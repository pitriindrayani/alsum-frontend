...
export default function DataTingkatan() {
  document.title = "Data Tingkatan";
  ...
  const GetResponseData = async () => {
    try {
      // e.preventDefault();
      setLoading(true)
      const response = await APIMS.get(`api/grade-levels?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}`,fetchParams)
  ...
  const deleteById = async (id) => {
    Swal.fire({
      ...
      if(result.isConfirmed) {
        const response =  await APIMS.delete(`api/grade-levels/${id}`,fetchParams);
  ...
  const viewModalUpdate = (id, kelas) => {
    setModalUpdate(true)
    setId(id)
    setDataUpdate(kelas)
  }
        ...
        {isTabletOrMobile ? 
            <>
                <div className="title-page">
                  <h6> <FontAwesomeIcon icon={faListUl} /> Data Tingkatan</h6>
                </div> 
                
                <div className="ml-auto">
                  <button onClick={viewModalAdd} className="btn btn-create"> <FontAwesomeIcon icon={faPlus} />Tingkatan</button>
                </div>
            </>
            : 
            <>
                <div className="title-page">
                  <h5> <FontAwesomeIcon icon={faListUl} /> Data Tingkatan </h5>
                </div>
          
                <div className="ml-auto">
                  <button onClick={viewModalAdd} className="btn btn-create"> <FontAwesomeIcon icon={faPlus} /> Tambah Tingkatan</button>
                </div>
            </>
        }
      </div> 
                ...
                <div className="body-table" >
                    <div>
                        <table className="table dt-responsive nowrap w-100" id="basic-datatable">
                            ...
                            <tbody>
                                {getData.map((col,index) => (
                                    <tr key={index}>
                                        <td style={{ lineHeight: "2" }}> {(page - 1) * 10 + (index + 1)} </td>  
                                        <td style={{ lineHeight: "2" }}> {col.name_level} </td>
                                        <td style={{ lineHeight: "2" }}> {col.name_branch} </td>
                                        <td style={{ lineHeight: "2" }}> {col.name_school} </td>
                                        <td>
                                            <button className="button-edit button-table" onClick={() => viewModalUpdate(col?.id, col)} > <FontAwesomeIcon icon={faPenToSquare} /></button>
                                            <button className="ml-2 button-delete button-table" onClick={() => {deleteById(col?.id)}}> <FontAwesomeIcon icon={faTrash} /></button> 
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
