...
export default function Jenjang() {
  ...
  const deleteById = async (id) => {
    Swal.fire({
      ...
      if(result.isConfirmed) {
        const response =  await APIMS.delete(`/api/educational-stages/${id}`,fetchParams);
  ...
  const viewModalUpdate = (id, jenjang) => {
    setModalUpdate(true)
    setId(id)
    setDataUpdate(jenjang)
  }
  return (
    <div className="body" >
        ...
        {isTabletOrMobile ? 
          <>
            <div className="title-page">
              <h6> <FontAwesomeIcon icon={faUsers} /> Data Jenjang</h6>
            </div> 
          ...
          <>
            <div className="title-page">
              <h5> <FontAwesomeIcon icon={faUsers} /> Data Jenjang </h5>
            </div>
          
            <div className="ml-auto">
              <button onClick={viewModalAdd} className="btn btn-create"> <FontAwesomeIcon icon={faPlus} /> Tambah Jenjang</button>
            </div>
          </>
        }
      </div> 
            ...
            <div className="body-table" >
              <div >
                <table className="table dt-responsive nowrap w-100" id="basic-datatable">
                  ...
                  <tbody>
                    {getData.map((jenjang,index) => (
                      <tr key={index}>
                        <td style={{ lineHeight: "2" }}> {(page - 1) * 10 + (index + 1)} </td>  
                        <td style={{ lineHeight: "2" }}> {jenjang.stages} </td>
                        <td style={{ lineHeight: "2" }}> {jenjang.stages_name} </td>
                        <td className="text-center" style={{ lineHeight: "2" }}> {jenjang.seq} </td>
                        <td className="text-center" style={{ lineHeight: "2" }}> {jenjang.min_grade} </td>
                        <td className="text-center" style={{ lineHeight: "2" }}> {jenjang.max_grade} </td>
                        <td>
                          <button className="button-edit button-table" onClick={() => viewModalUpdate(jenjang?.id, jenjang)} > <FontAwesomeIcon icon={faPenToSquare} /></button>
                          <button className="ml-2 button-delete button-table" onClick={() => {deleteById(jenjang?.id)}}> <FontAwesomeIcon icon={faTrash} /></button>                           
                        </td>
                      </tr>
                    ))}
                  </tbody>
