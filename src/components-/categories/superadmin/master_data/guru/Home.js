...
export default function Guru() {
  ...
  const postDataByExcel = async () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          setLoading(true)
          const response = await APIUS.post(`/api/teachers/excel/store`,{ file: data },fetchParams);
  ...
  const GetResponseData = async () => {
    try {
      // e.preventDefault();
      setLoading(true)
      const response = await APIUS.get(`/api/teachers?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}&branch=${storageBranch}&level=${storageLevel}`,fetchParams)
  ...
  const deleteById = async (id) => {
    Swal.fire({
      ...
      if(result.isConfirmed) {
        const response =  await APIUS.delete(`/api/teachers/${id}`,fetchParams);
        if (response.data.error == false) {
          GetResponseData()
          swal({
            title: 'Success',
        ...
        {isTabletOrMobile ? 
          <>
            <div className="title-page">
              <h6> <FontAwesomeIcon icon={faUsers} /> Data Guru</h6>
            </div> 
          ...
          <>
            <div className="title-page">
              <h5> <FontAwesomeIcon icon={faUsers} /> Data Guru </h5>
            </div>
            ...
            <div className="body-table" >
              <div>
                <table className="table dt-responsive nowrap w-100" id="basic-datatable">
                  ...
                  <tbody>
                    {getData.map((guru,index) => (
                      <tr key={index}>
                        <td style={{ lineHeight: "2" }}> {(page - 1) * 10 + (index + 1)} </td>  
                        <td style={{ lineHeight: "2" }}> {guru?.nip_ypi_karyawan === null ? guru?.nip_ypi : guru?.nip_ypi_karyawan} </td>
                        <td style={{ lineHeight: "2" }}> {guru.full_name} </td>
                        <td style={{ lineHeight: "2" }}> {guru?.user_detail?.email} </td>
                        <td style={{ lineHeight: "2" }}> {guru.ysb_school_id} </td>
                        <td style={{ lineHeight: "2" }}> {guru.ysb_branch_id} </td>
                        <td>
                          <button className="button-edit button-table" onClick={() => viewModalUpdate(guru?.id, guru)} > <FontAwesomeIcon icon={faPenToSquare} /></button>
                          <button className="ml-2 button-delete button-table" onClick={() => {deleteById(guru?.id)}}> <FontAwesomeIcon icon={faTrash} /> </button>                           
                        </td>
                      </tr>
                    ))}
                  </tbody>
