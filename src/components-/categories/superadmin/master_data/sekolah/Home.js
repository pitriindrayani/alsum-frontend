...
export default function Sekolah() {
  ...
  const GetResponseData = async () => {
    try {
      // e.preventDefault();
      setLoading(true)
      const response = await APIMS.get(`/api/schools?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}`,fetchParams)
  ...
  const deleteById = async (id) => {
    Swal.fire({
      ...
      if(result.isConfirmed) {
        const response =  await APIMS.delete(`/api/schools/${id}`,fetchParams);
                  ...
                  <tbody>
                      {getData.map((school,index) => (
                        <tr key={index}>
                          ...
                          <td style={{ lineHeight: "2" }}> {school.npsn} </td> 
