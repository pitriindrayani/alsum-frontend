...
export default function Pendaftaran() {
  ...
  const deleteById = async (id) => {
    Swal.fire({
      ...
      if(result.isConfirmed) {
        const response =  await APIUS.delete(`/api/students/${id}`,fetchParams);
  ...
  const viewModalUpdate = (id, siswa) => {
    setModalUpdate(true)
    setId(id)
    setDataUpdate(siswa)
  }
        ...
        {isTabletOrMobile ? 
          <>
            <div className="title-page">
              <h6> <FontAwesomeIcon icon={faUsers} /> Data Siswa</h6>
            </div> 
                
            <div className="ml-auto">
              <button onClick={handleShow} className="btn btn-create"> <FontAwesomeIcon icon={faUserPlus} />User</button>
            </div>
          </>
          : 
          <>
            <div className="title-page">
              <h5> <FontAwesomeIcon icon={faUsers} /> Data Siswa </h5>
            </div>
          
            <div className="ml-auto">
              <button onClick={viewModalAdd} className="btn btn-create"> <FontAwesomeIcon icon={faUserPlus} /> Tambah Siswa</button>
            </div>
          </>
        }
            ...
            <div className="body-table" >
              <div>
                <table className="table dt-responsive nowrap w-100" id="basic-datatable">
                  ...
                  <tbody>
                    {getData.map((siswa,index) => (
                      <tr key={index}>
                        <td style={{ lineHeight: "2" }}> {(page - 1) * 10 + (index + 1)} </td>  
                        <td style={{ lineHeight: "2" }}> {siswa.nik} </td>
                        <td style={{ lineHeight: "2" }}> {siswa.nama_lengkap} </td>
                        <td style={{ lineHeight: "2" }}> {siswa.email_murid} </td>
                        <td>
                          <button className="button-edit button-table" onClick={() => viewModalUpdate(siswa?.id, siswa)} > <FontAwesomeIcon icon={faPenToSquare} /></button>
                          <button className="ml-2 button-delete button-table" onClick={() => {deleteById(siswa?.id)}}> <FontAwesomeIcon icon={faTrash} /></button>                           
                        </td>
                      </tr>
                    ))}
                  </tbody>
