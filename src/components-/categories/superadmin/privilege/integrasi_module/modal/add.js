...
export default function ModalRoleAdd(props) {
  ...
  const [getDataModule, setGetDataRoom] = useState([]);
  ...
  const GetResponseData = async () => {
    try {
      setLoading(true)
      // e.preventDefault();
      const response = await APIUS.get(`/api/privileges/modules?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}&search=${search}`,fetchParams)
      // Checking process
  ...
  const handleSubmit = useMutation(async (e) => {
    try {
      ...
      const response = await APIUS.post("/api/privileges/app-modules/store",{
        id_app: props.idAdd,
        modules: inputList,
      },fetchParams);
      // Checking process
      ...
      <Modal.Body className="modal-body">
        <Form  onSubmit={(e) => handleSubmit.mutate(e)}> 
           {inputList.map((x, i) => {
            ...
            <div>
                <div className="mt-2" >
                    <label className="label-form" >Pilih Module </label>
                    <select aria-label="Default select example" onChange={(e) => handleInputChange(e, i, "id_module")}  name="id_module" className="select-form" >
                        <option value="" hidden>Pilih Module..</option>
                        {getDataModule.map((module,index) => (
