...
export default function ModalRoleUpdate(props) {
  ...
  const GetResponseData = async () => {
    try {
      // e.preventDefault();
      setLoading(true)
      const response = await APIUS.get(`/api/privileges/app-modules?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}&search=${search}&id_app=${props.idTrash}`,fetchParams)
  ...
  const handleSubmit = useMutation(async (e) => {
    try {
      ...
      const payload = {
        modules: formSubModule?.subModule.map(item => ({ id_module: item.id_module })),
      };
  
      // permintaan DELETE dengan payload
      const response = await APIUS.delete(`/api/privileges/app-modules/${props.idTrash}`, {
        data: payload,
        ...fetchParams, 
      });
  ...
  const handleChangeCheckbox = (e) => {
    ...
    if (checked) {
      setFormTreatment({
        subModule: [...subModule, {"id_module" : `${value}`}],
      });
    }
    ...
    // Case 2  : The user unchecks the box
      ...
      setFormTreatment({
        subModule: subModule.filter(e => {return e.id_module !== `${value}`} ),
                    ...
                    <div className="mt-3" style={{fontSize: "14px"}}>
                        {getDataModule.map((user,index) => (
                            <div className="mb-2" key={index} style={{display:"flex", width:"100%", cursor:"pointer"}}>
                                <input className="mr-2" autoFofcus type='checkbox' value={user?.id_module} name="id_module" onChange={handleChangeCheckbox}  />
                                <div>
