...
export default function IntegrasiMenu() {
  document.title = "Integrasi Menu";
  ...
  //Get ID
  const {id_module, slug_name} = useParams();
  ...
  const GetResponseData = async () => {
    try {
      // e.preventDefault();
      setLoading(true)
      const response = await APIUS.get(`/api/privileges/module-menus?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}&id_module=${id_module}`,fetchParams)
  ...
  const modalMedisRoleTrash = () => {
    ...
    setIdTrash(id_module)
  }

  const navigateSubModules = (id,slug_name) => {
    navigate("/privileges/apps/" + id  + "/" + slug_name)
  }
      ...
      <div className="body-header d-flex">
          ...
          <>
            <div className="title-page">
              <h6> <FontAwesomeIcon icon={faBars} /> Integrasi Menu {slug_name} </h6>
            </div> 
          ...
          <>
            <div className="title-page">
              <h5><FontAwesomeIcon icon={faBars} /> Integrasi Menu <span style={{textTransform: "capitalize"}}>(Module : {slug_name})</span></h5>
            </div>
