...
export default function Login() {
  ...
  const GetResponseData = async () => {
    try {
      // e.preventDefault();
      // setLoading(true)
      const response = await APIUS.get(`/api/privileges/menus?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}`,fetchParams)
  ...
  const deleteById = async (id) => {
    Swal.fire({
      ...
      if(result.isConfirmed) {
        const response =  await APIUS.delete(`/api/privileges/menus/${id}`,fetchParams);
