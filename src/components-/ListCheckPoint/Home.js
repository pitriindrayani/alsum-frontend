...
import ModalScanMe from "./ModalAdmin/ModalScanMe";
...
export default function ListCheckPoint(props) {
  ...
  const [modalScanMe, setModalScanMe] = useState(false);
  ...
  const GetResponseData = async () => {
    try {
      // e.preventDefault();
      setLoading(true)
      const response = await APIMS.get(`/api/check-points?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}`,fetchParams)
  ...
  return (
    <div className="body" >
      ...
      {modalScanMe  && <ModalScanMe  show={modalScanMe} onHide={() => setModalScanMe(false)} />}
