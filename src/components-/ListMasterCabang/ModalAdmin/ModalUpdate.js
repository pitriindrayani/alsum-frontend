...
export default function ModalRoleUpdate(props) {
  ...
  const GetResponseData = async () => {
    const response = await API.get(`/user-service/branches?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}`,fetchParams)
  ...
  const handleSubmit = useMutation(async (e) => {
    try {
      ...
      const response = await API.put(`/user-service/branches/${props.id}`, {
        ...
        <Form onSubmit={(e) => handleSubmit.mutate(e)} className="mt-3" >
          <div className="" style={{ display: "flex"}}>
            <div style={{ display:"flex", width:"100%"}}>
              <select ref={nameInputRef} aria-label="Default select example" value={form?.parent_id} onChange={handleChange}  name="parent_id" style={{color:"#2e649d", textAlign:"", 
                cursor:"pointer", border:"2px solid #2e649d",width:"100%", height:"45px", borderRadius:"5px"}}>
                <option value="" hidden>ID Parent ..</option>
          ...
          <div className="mt-4" style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', 
            ...
            <input autoFocus type='text' name="branch_code" onChange={handleChange} value={form?.branch_code}  
          ...
          <div className="mt-4" style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', 
            ...
            <input type='text' name="branch_name" onChange={handleChange} value={form?.branch_name}  
              placeholder='Masukan Nama Menu' style={{ backgroundColor: 'transparent', border: 'none', 
              outline: 'none', color: '#818181', padding: '5px 5px 5px 10px', flex: 1, fontSize: '14px',width:"300px" }}/>
            <style>{`input::placeholder { color: #B9B9B9;}`}
            </style>
