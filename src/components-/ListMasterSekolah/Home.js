...
export default function Login() {
  ...
  const [stagesUpdate, setStagesUpdate] = useState();
  const [stagesNameUpdate, setStagesNameUpdate] = useState();
  const [seqUpdate, setSeqUpdate] = useState();
  const [minGradeUpdate, setMinGradeUpdate] = useState();
  const [maxGradeUpdate, setMaxGradeUpdate] = useState();
  // modal add
...
useEffect(() => {
  ...
  const GetResponseData = async () => {
    try {
      // e.preventDefault();
      // setLoading(true)
      const response = await API.get(`/user-service/educational-stages?page=${page}&limit=${limit}&ascending=${ascending}&search=${keyword}`,fetchParams)
  ...
  const deleteById = async (id) => {
    Swal.fire({
      ...
      if(result.isConfirmed) {
        const response =  await API.delete(`/api/educational-stages/${id}`,fetchParams);
  ...
  const viewModalUpdate = (id, stages, stages_name, seq, min_grade, max_grade) => {
    setModalUpdate(true)
    setId(id)
    setStagesUpdate(stages)
    setStagesNameUpdate(stages_name)
    setSeqUpdate(seq)
    setMinGradeUpdate(min_grade)
    setMaxGradeUpdate(max_grade)
  }
  ...
  return (
    <div style={{ backgroundColor: "white", margin: "15px", marginRight: "10px", boxShadow: "2px 2px 10px #BFBFBF" }}>
      ...
      {modalUpdate && <ModalUpdateMedis GetResponseData={GetResponseData} stagesUpdate={stagesUpdate} stagesNameUpdate={stagesNameUpdate} seqUpdate={seqUpdate} 
       minGradeUpdate={minGradeUpdate} maxGradeUpdate={maxGradeUpdate} id={id} show={modalUpdate} onHide={() => setModalUpdate(false)} />}
      {loading && <LoaderHome />}
      ...
      {isTabletOrMobile ? 
        <div style={{ paddingLeft: "0px", width: "100%", borderBottom: "5px solid #EEEEEE", display: "", padding: "0px 0px 10px 0px" }}>
          <Col xl="6" style={{fontSize:"16px",display:"flex", justifyContent:"center", alignItems:"center", padding:"7px", color:"white", backgroundColor:"#2e649d"}}>
              <FaListAlt style={{marginRight:"5px"}}/>List Sekolah
          </Col>
            ...
            <table className="table dt-responsive nowrap w-100" id="basic-datatable">
              <thead>
                <tr style={{backgroundColor: isTabletOrMobile? "white" : "white", borderBottom:"1px solid rgb(214, 214, 214)"}}>
                  ...
                  <th style={{ fontFamily: "Poppins", fontSize: "12px", color: "#525252", border: "none", textAlign: ""}}>KODE SEKOLAH</th>
                  <th style={{ fontFamily: "Poppins", fontSize: "12px", color: "#525252", border: "none", textAlign: ""}}>JENJANG</th>
                  <th style={{ fontFamily: "Poppins", fontSize: "12px", color: "#525252", border: "none", textAlign: "center"}}>TAHAP</th>
                  <th style={{ fontFamily: "Poppins", fontSize: "12px", color: "#525252", border: "none", textAlign: "center"}}>AWAL KELAS</th>
                  <th style={{ fontFamily: "Poppins", fontSize: "12px", color: "#525252", border: "none", textAlign: "center"}}>AKHIR KELAS</th>
                    ...
                    <tr key={index} style={{fontFamily:"Poppins", fontSize:"11px", textAlign:"center"}}>
                        ...
                        <div style={{display:"flex", alignItems:"center", justifyContent:"center"}}>
                          <div>
                            <div style={{  display:"flex", justifyContent:"center" }}>
                              <div style={{ borderRadius:"3px", fontWeight:"bold",border:"1px solid #FFB450", color:"black", padding:"5px 20px"}}>
                              {user?.seq}
                              </div>
                            </div>
                          </div> 
                      ...
                      <td style={{ lineHeight: "2" }}>
                        <div style={{display:"flex", alignItems:"center", justifyContent:"center"}}>
                          <div>
                            <div style={{  display:"flex", justifyContent:"center" }}>
                              <div style={{ borderRadius:"3px", fontWeight:"bold",backgroundColor:"rgb(30, 193, 161)", color:"white", padding:"5px 20px"}}>
                              {user?.min_grade}
                              </div>
                            </div>
                          </div> 
                      ...
                      <td style={{ lineHeight: "2" }}>
                        <div style={{display:"flex", alignItems:"center", justifyContent:"center"}}>
                          <div>
                            <div style={{  display:"flex", justifyContent:"center" }}>
                              <div style={{ borderRadius:"3px", fontWeight:"bold",backgroundColor:"rgb(141, 80, 255)", color:"white", padding:"5px 20px"}}>
                              {user?.max_grade}
                              </div>
                            </div>
                          </div> 
                      ...
                      <td style={{lineHeight:"1"}}>
                        <div style={{display:"flex",justifyContent:"center", alignItems:"center", alignContent:"center"}}>
                            {permission.update === 1 ?
                            <button onClick={() => viewModalUpdate(user?.id, user?.stages, user?.stages_name, user?.seq, user?.min_grade, user?.max_grade)} style={{ fontSize: "17px", color: "#2196F3", backgroundColor: "white", borderRadius: "3px", cursor: "pointer", border: "none", display:"flex", marginRight:"3px" }}>
                              <FaCog/>
                            </button>:""}
                            {permission.delete === 1 ?
